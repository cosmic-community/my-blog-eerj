...
// Normalize tags which may come back as a string or an array
export function getTags(field: unknown): string[] {
  if (!field) return []
  if (Array.isArray(field)) {
    return field.map((t) => String(t)).filter(Boolean)
  }
...
export async function getAllPosts(): Promise<Post[]> {
  try {
    const response = await cosmic.objects
      ...
      .depth(1)
    return sortByNewest(response.objects as Post[])
  } catch (error) {
